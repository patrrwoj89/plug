...
- **Wycentrowane przewijanie focusu na Android TV** (`TvBringIntoViewSpec`, `TvBringIntoViewProvider`): niestandardowy `BringIntoViewSpec` z płynną animacją `SpringSpec` centruje zaznaczony `MediaCard` w `LazyRow` i `LazyVerticalGrid`/`LazyColumn` (`CategoryRow`, `LibraryScreen`, `WatchlistScreen`, `CustomListDetailScreen`, `CustomListsScreen`), więc nawigacja D-Padem nie zostawia aktywnego kafelka przy krawędzi ekranu.
...
- **Spoiler Blur** — opisy, plakat i tytuł nieobejrzanych odcinków na `DetailScreen` są rozmyte/ukryte (`Modifier.blur(16.dp)`). Tytuł zastępowany jest lokalizowanym tekstem `Odcinek X` / `Ukryty tytuł`, a D-Pad Center/SELECT na kafelku odsłania tytuł, plakat i opis na czas bieżącego seansu.
...
- **Wygaszacz OLED** (`OledBurnInSaver`, `TVNavHost`): po 5 minutach bezczynności D-Pada, gdy żadne wideo nie jest odtwarzane, ekran przyciemnia się o 85% i wyświetla minimalistyczny zegar cyfrowy oraz wolno poruszające się okładki z cache Coil. Dowolny przycisk pilota natychmiast przywraca pełną jasność.
...
- **Podręczne menu szybkich ustawień playera** (`PlayerQuickSettingsOverlay`, `PlayerControls`, `PlayerViewModel`): ikona zębatki w kontrolkach odtwarzacza otwiera dolną nakładkę z przełącznikami trybu nocnego (`LoudnessEnhancer`), preferencji audio (`Lektor` / `Dubbing`) oraz silnika (`ExoPlayer` / `LibVLC`). Stan nakładki subskrybowany jest przez `collectAsStateWithLifecycle()`, a zamknięcie zwraca focus na suwak postępu.
- **Webhooki Home Assistant** (`HomeAssistantWebhookClient`, `PlayerViewModel`): konfigurowalny adres Home Assistant i token webhooka w `Ustawieniach` / panelu admina; zdarzenia `play`, `pause` i `stop` są wysyłane asynchronicznie na `Dispatchers.IO` jako `POST /api/webhook/{token}` z bieżącym profilem i tytułem mediów. URL i token webhooka nie są logowane w buildach release.
- **Wewnętrzny mini-player wideo** (`VideoPipManager`, `InAppVideoPipOverlay`): wciśnięcie WSTECZ z odtwarzacza zachowuje instancję ExoPlayer, wysuwa mini-odtwarzacz w prawym dolnym rogu ekranu głównego i kontynuuje wideo/audio. Środkowy przycisk D-Pada przywraca pełny ekran; Stop zwalnia playera i czyści pamięć.
- **Tuning natywnego silnika ExoPlayer** (`ExoPlayerTuningConfig`, `PlayerScreen`, `NetworkModule`): odtwarzanie tunelowane przez `DefaultTrackSelector.Parameters.setTunnelingEnabled`, pre-alokowany `DefaultAllocator`/`DefaultLoadControl` z konfigurowalnym min/max/playback/back buffer, początkową liczbą alokacji i target buffer size. Równoległe połączenia HTTP są aplikowane do dispatcher-a globalnego `OkHttpClient`.
- **Reguły Debrid / TorBox Stream Rules** (`StreamRulesEngine`, `CompositeMediaRepository`): JSON `@Serializable` `StreamRules` przechowywany w DataStore filtruje źródła po zakresie rozmiaru, rozdzielczości, wymaganych/preferowanych/wykluczonych tagach wideo/audio (HDR, Dolby Vision, Atmos, DTS itp.) i enkoderach (HEVC, AV1, H264). Silnik parsuje tytuły/podtytuły/opisy, a `CompositeMediaRepository` aplikuje reguły do wyników wyszukiwania i szczegółów.
- **Binge Grouping** (`PlayerViewModel`): gdy użytkownik ręcznie wybierze profil źródła dla odcinka serialu, w pamięci RAM tworzony jest `BingeProfile` na podstawie metadanych strumienia i ponownie używany do rankingu/wyboru tego samego profilu dla kolejnych odcinków sezonu.
- **Tryb AMOLED / Pure Black** (`TVHubTheme`, `MainActivity`, `SettingsScreen`): `AMOLED Mode` wymusza czyste czarne tło (`#000000`), a `Pure Black Surfaces` ustawia również karty/warstwy surface na czystą czerń.
...
- **Konsola deweloperska / Piaskownica wtyczek** w bezprzewodowym panelu admina. Strona ładuje CodeMirror z cdnjs (numery linii i kolorowanie składni). Endpoint `POST /api/plugin/test?format=js|json|python` pozwala testować fragmenty QuickJS (globalne `httpFetch`/`httpFetchText`), walidować payloady JSON `MediaItem` i wysyłać skrypty `.py` do skonfigurowanej instancji Kodi (`Files.WriteFile` + `XBMC.RunScript`) bezpośrednio z telefonu.
...
- **Ustawienia tuningu silnika ExoPlayer**: odtwarzanie tunelowane, połączenia równoległe (1–16), min/max/playback/back buffer, początkowa liczba alokacji i target buffer size można ustawić w `Ustawienia → Wygląd i Odtwarzanie Premium` lub w bezprzewodowym panelu admina.
- **Ustawienia Stream Rules**: JSON reguł Debrid/TorBox można edytować z `Ustawienia → Wygląd i Odtwarzanie Premium` lub przez endpoint `/api/config` w panelu admina.
- **Tryb AMOLED / Pure Black Surfaces** oraz przełącznik **Binge Grouping** w `Ustawienia → Wygląd i Odtwarzanie Premium` i w bezprzewodowym panelu admina.
...
- **Hartowanie produkcyjne / final code freeze**: wszystkie wywołania `Log` są opakowane przez `BuildConfig.DEBUG`; w buildach release do Logcat nie trafiają klucze API, tokeny webhooków, URL-e, linki źródeł, parametry filtrów ani zapytania użytkownika. `LevenshteinEngine` normalizuje polskie znaki diakrytyczne przez NFD, `HomeAssistantWebhookClient` używa defensywnych timeoutów 3 s i zamyka `response.body` w `finally`, a `UniversalVlcPlayer` aplikuje Binge Grouping audio/jakość na równi z ExoPlayerem. Reguła ProGuard `-assumenosideeffects` wycina wywołania `Log.d`/`Log.v`/`isLoggable` z produkcyjnego `.apk`.
