...
- **Centered TV focus scrolling** (`TvBringIntoViewSpec`, `TvBringIntoViewProvider`): a custom `BringIntoViewSpec` with a smooth `SpringSpec` centers the focused `MediaCard` in `LazyRow` and `LazyVerticalGrid`/`LazyColumn` (`CategoryRow`, `LibraryScreen`, `WatchlistScreen`, `CustomListDetailScreen`, `CustomListsScreen`) so D-Pad navigation never leaves the active tile pinned at the screen edge.
...
- **Spoiler Blur**: unwatched episode plot descriptions, poster and episode title are blurred/hidden on `DetailScreen`. The title is replaced with a localized `Episode X` / `Hidden title` placeholder and D-Pad Center/SELECT on the tile reveals title, poster and description for the current session.
...
- **OLED Burn-In Saver** (`OledBurnInSaver`, `TVNavHost`): after 5 minutes of D-Pad inactivity while no video is playing, the screen dims by 85% and shows a minimalist clock plus slowly moving posters from Coil cache. Any key press restores full brightness.
...
- **Player quick settings overlay** (`PlayerQuickSettingsOverlay`, `PlayerControls`, `PlayerViewModel`): a gear icon in the player controls opens a bottom overlay with D-Pad toggles for Night mode (`LoudnessEnhancer`), audio preference (`Lektor` / `Dubbing`) and player engine (`ExoPlayer` / `LibVLC`). Overlay state uses `collectAsStateWithLifecycle()` and closing it returns focus to the progress `Slider`.
- **Home Assistant Smart Cinema webhooks** (`HomeAssistantWebhookClient`, `PlayerViewModel`): configurable Home Assistant URL and webhook token in `Settings` / admin panel; `play`, `pause` and `stop` events are sent asynchronously on `Dispatchers.IO` as `POST /api/webhook/{token}` with the current profile and media title. Webhook URL and token are never logged in release builds.
- **In-App Video Picture-in-Picture** (`VideoPipManager`, `InAppVideoPipOverlay`): pressing BACK from the player preserves the ExoPlayer instance, slides up a mini-player in the bottom-right corner of the home screen and keeps video/audio playing. D-Pad Center/SELECT restores full-screen playback; Stop releases the player and clears memory.
- **ExoPlayer native engine tuning** (`ExoPlayerTuningConfig`, `PlayerScreen`, `NetworkModule`): tunneled playback via `DefaultTrackSelector.Parameters.setTunnelingEnabled`, pre-allocated `DefaultAllocator`/`DefaultLoadControl` with configurable min/max/playback/back buffer, initial allocation count and target buffer size. Parallel HTTP connections are applied to the global `OkHttpClient` dispatcher.
- **Debrid / TorBox Stream Rules** (`StreamRulesEngine`, `CompositeMediaRepository`): a `@Serializable` `StreamRules` JSON stored in DataStore filters sources by size range, resolution, required/preferred/excluded video/audio tags (HDR, Dolby Vision, Atmos, DTS, etc.) and encoders (HEVC, AV1, H264). The engine parses titles/subtitles/descriptions and `CompositeMediaRepository` applies the rules to search and detail results.
- **Binge Grouping** (`PlayerViewModel`): when a user manually selects a source profile for a series episode, an in-memory `BingeProfile` is built from the stream metadata and reused to rank/select the same profile for all remaining episodes in the season.
- **AMOLED / Pure Black mode** (`TVHubTheme`, `MainActivity`, `SettingsScreen`): `AMOLED Mode` forces a pure black (`#000000`) background and `Pure Black Surfaces` also sets cards/surface layers to pure black.
...
- **ExoPlayer native engine settings**: tunneled playback, parallel connections (1–16), min/max/playback/back buffer, initial allocation count and target buffer size can be set from `Settings → Appearance & Playback Premium` or the wireless admin panel.
- **Stream Rules settings**: Debrid/TorBox stream rules JSON can be edited from `Settings → Appearance & Playback Premium` or the wireless admin panel `/api/config`.
- **AMOLED Mode / Pure Black Surfaces** and **Binge Grouping** toggles in `Settings → Appearance & Playback Premium` and the wireless admin panel.
...
- **Production hardening / final code freeze**: all `Log` calls are guarded with `BuildConfig.DEBUG`; no API keys, webhook tokens, URLs, source links, filter parameters or user search queries are written to Logcat in release builds. `LevenshteinEngine` normalizes Polish diacritics with NFD, `HomeAssistantWebhookClient` uses defensive 3-second timeouts and explicitly closes response bodies, and `UniversalVlcPlayer` applies Binge Grouping audio/quality profiles in parity with ExoPlayer. A ProGuard `-assumenosideeffects` rule strips `Log.d`/`Log.v`/`isLoggable` calls from the release `.apk`.
